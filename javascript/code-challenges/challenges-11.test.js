...
function transformToLis(obj){
  ...
  const items = [];
  for (let key in obj) {
    const listItems = `<li>${key}: ${obj[key]}</li>`;
...
let findMaleAndFemale = (data) => {
  ...
  const characterNames = maleAndFemaleCharacters.map((character) => character.name);
  return characterNames.join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  const shortestCharacter = data.reduce((shortest, character) => {
